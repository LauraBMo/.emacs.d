...
(defun brust-LaTeX-insert-_ nil (interactive) (insert "_{}") (forward-char -1))
(defun brust-LaTeX-insert-^ nil (interactive) (insert "^{}") (forward-char -1))
...
#+BEGIN_SRC elisp
  ...
  (defun brust-cdlatex-tab-close-parent-in-math-mode nil
    (interactive)
    (when (texmathp)
      (narrow-to-region (+ (cdr texmathp-why)
                           (length (car texmathp-why)))
                        (point))
      (end-of-buffer)
      (let ((-closed-p (ignore-errors (close-quoted-open-paren-right nil))))
        (widen)
        -closed-p)))
...
#+BEGIN_SRC elisp
  (mapc (lambda (-key)
          (setq cdlatex-env-alist-default
                (brust-list-delete-element -key cdlatex-env-alist-default)))
        '(#("array" 0 1 (idx 13))
          #("deflist" 0 1 (idx 15))
...
#+BEGIN_SRC elisp
  (mapc (lambda (-key)
          (setq cdlatex-command-alist-default
                (brust-list-delete-element -key cdlatex-command-alist-default)))
        '("pref"
          "ct"
          "cte"
          "cite{"
