...
* TODO LaTeX-init
** COMMENT To use accents as char itself.
...
#+BEGIN_SRC elisp
 ...
 ;; ::TODO Mirà que conecta el customize-map-.... de outline. 
  ...
  ;;(add-hook 'LaTeX-mode-hook 'paren-toggle-matching-quoted-paren 1 t)
  ;;(add-hook 'LaTeX-mode-hook 'paren-toggle-matching-paired-delimiter 1 t)
  ;;(add-hook 'LaTeX-mode-hook 'imenu-add-menubar-index)

  ;;(add-hook 'LaTeX-mode-hook 'TeX-fold-mode  ;Ocultar seccions, mostrar simbols "fold the LaTeX macros")
  ;; This needs (setq TeX-fold-auto t)
...
** Never open a file in TeX-mode, use LaTeX-mode
#+BEGIN_SRC elisp
...
** Save options
#+BEGIN_SRC elisp
     (setq TeX-save-query    nil ;; Don't query on TeX-save-document "C-c C-d" save all files of the document.
...
#+END_SRC
** COMMENT Electric
*** COMMENT If non-nil, insert braces after typing _ and ^ in math mode
#+BEGIN_SRC elisp
(setq TeX-electric-sub-and-superscript nil)

#+END_SRC
*** COMMENT Autocomplet braces/parents 
#+BEGIN_SRC elisp
...
#+BEGIN_SRC elisp
  (setq reftex-plug-into-AUCTeX t ;; To integrate RefTeX with AUCTeX
        ...
        reftex-default-bibliography '("~/Dropbox/bibliografy/my.bib"
                                      "~/Dropbox/bibliografy/zotero-bibl.bib")    
...
#+BEGIN_SRC elisp
  ...
  (defun brust-get-path-to-a-file-from-TeX-master (file)
    (save-excursion 
      (goto-char (point-min))
      (re-search-forward (concat "include{\\(.*" file "\\)}") nil t)
        ...
        (let ((init (buffer-name))
          ...
          (save-excursion
            (let ((latex/view-after-compile nil)
                  (path-to-file (brust-get-path-to-a-file-from-TeX-master init))
              ...
              (brust-restore-buffer-to text-buffer)))
          (set-buffer init)
...
#+BEGIN_SRC elisp
  ...
  (setq brust/LaTeX/init-dir "~/.emacs.d/LaTeX")
#+END_SRC
...
****** Names envoirments
#+BEGIN_SRC elisp
(defconst environment-prefix-list '(("corollary" . "coro") ("notation" . "not") ("lemma" . "lem")
...
#+BEGIN_SRC elisp
  (cl-loop
   for xx in
   '(
     ("apl"   "Insert \\(\\apl...\\)" "\\(\\apl{?}{}\\)"                cdlatex-position-cursor   nil t   nil)
     ("oapl"   "Insert \\(\\overapl...\\)" "\\(\\overapl{?}{}{}\\)"     cdlatex-position-cursor   nil t   nil)
     ("uapl"   "Insert \\(\\underapl...\\)" "\\(\\underapl{?}{}{}\\)"   cdlatex-position-cursor   nil t   nil)
...
#+BEGIN_SRC elisp
  ...
  (defun brust/cdlatex-free-keys nil
    (define-key cdlatex-mode-map (kbd "'") nil)
    (define-key cdlatex-mode-map (kbd "`") nil))

  (add-hook 'LaTeX-mode-hook 'brust/cdlatex-free-keys t)
  ...
  ;;
  ;;   (defadvice cdlatex-dollar (around new-math activate)
  ;;     ad-do-it
  ;;     (save-excursion 
  ;;       (when (texmathp)
  ...
  ;;    (re-search-forward (format "%s\([.]+\)%s" (car texmathp-why) (car texmathp-why)) nil t)
  ;;    (replace-match 
  ...
  ;;          (replace-match "\[")
  ;;        (when (search-backward "$" (- (point) 100) t)
  ;;          (replace-match "\\(")))
  ;;      (if (search-forward "$$" (- (point) 100) t)
  ;;          (replace-match "\]")
  ;;        (when (search-forward "$" (+ (point) 100) t)
  ;;          (replace-match "\\)")))))


  ;;(defadvice cdlatex-math-symbol (around use-out-math activate)
  ;;  (if (texmathp) ad-do-it
  ;;    (cdlatex-math-modify)))
  ;;
  ;;(defadvice cdlatex-math-modify (around use-only-in-math activate)
  ;;  (if (texmathp) ad-do-it
  ;;    (insert "'")))

  ;;    (ad-unadvise 'cdlatex-math-symbol)
#+END_SRC
