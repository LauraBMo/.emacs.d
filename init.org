...
#+RESULTS:
| %e | (:propertize |   | face mode-line-bars-face) | (:propertize display-time-string face sml/line-number) |   | (:eval (cond (buffer-read-only (propertize  RO  (quote face) (quote mode-line-read-only-face))) ((buffer-modified-p) (propertize  **  (quote face) (quote mode-line-modified-face))) (t    ))) |   | (:propertize [ face mode-line-bars-face) | (:propertize %l face sml/line-number) | (:eval (propertize (save-excursion (goto-char (point-max)) (format-mode-line /%l)) (quote face) (quote mode-line-top-line-number))) | (:propertize : face sml/line-number) | (:eval (propertize %c (quote face) (if (< (current-column) 80) (quote sml/col-number) (quote mode-line-80col-face)))) | (:propertize ] face mode-line-bars-face) | mode-line-buffer-identification | (:propertize (  [ mode-name ] ) face sml/modes) | (:eval (my-mode-line-vc-info)) | (:propertize | face mode-line-bars-face) | (:eval (nyan-create)) | (:propertize |   | %- face mode-line-bars-face) | mode-line-end-spaces |
...
#+RESULTS:
**** Company-statistics
  ;; Company-statistics is a global minor mode keeping a log of completions,
  ;; along with some context information, and use that to rank candidates.
#+BEGIN_SRC elisp
...
#+BEGIN_SRC elisp
  ...
  (setq gap-executable "~/src/gap-4.10.2/bin/gap.sh")
...
#+begin_src elisp
  (defun brust-mu4e-rebuild-index (msg)
    (async-shell-command "~/src/mu/mu/mu index --rebuild"))
