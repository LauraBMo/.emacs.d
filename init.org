...
#+BEGIN_SRC elisp
  ...
  (setq ispell-program-name "aspell"
        ...
        ispell-personal-dictionary (file-truename "~/.emacs.d/lisp/english-words.txt")) ;; "~/.emacs.d/myDictionary.pws"
#+END_SRC
...
#+BEGIN_SRC elisp
  ...
  (use-package key-chord
    ...
    ;; (key-chord-define-global "jj" 'undo)
    ;; (key-chord-define-global "cj" 'brust-cycle-whitespace)
    (key-chord-define-global "bg" #'ivy-switch-buffer)
    (key-chord-define-global "bf" #'brust-ace-window)
    (key-chord-define-global "vv" #'hydra-outline/body)
    (key-chord-define-global "ww" #'zap-up-to-char)
    (key-chord-define-global ".s" #'flyspell-correct-previous-word-generic))
...
#+BEGIN_SRC elisp
  (bind-keys*
   ...
   ("M-j"        . undo)
   ("s-ç"        . endless/ispell-word-then-abbrev)
...
*** Elisp
#+BEGIN_SRC elisp
  (defun brust-er/add-elisp-mode-expansions ()
    "Adds expansions for buffers in elisp-mode"
  ...
  (add-hook 'emacs-lisp-mode-hook 
	    #'brust-er/add-elisp-mode-expansions)
#+END_SRC
*** Font lock.
#+BEGIN_SRC elisp
...
#+BEGIN_SRC elisp
  (defun my-texmathp nil
    (interactive)
    (when (texmathp)
      (when (string= (car texmathp-why) "\\(")
        (ignore-errors (scan-sexps (cdr texmathp-why) 1)))))
    ...
    (cl-loop repeat args do
             (if (and (fboundp 'texmathp)
                 ...
                 (unless (and (ignore-errors (close-quoted-open-paren-right 1))
                              (if (texmathp) t (delete-char -2) nil))
  ...
  (setq
   ...
   '((xah-filelist . (
     ...
     (config-files . (
                      ...
                      ("abbrevs-tbl"  . "~/.emacs.d/lisp/abbrev-defs.el")
                      ))
     (org-files    . (
...
#+BEGIN_SRC elisp
  (use-package tex 
    ...
    (:map LaTeX-mode-map 
          ("zp" . brust-LaTeX-save)
          ("zn" . org-ref-helm-insert-cite-link)
          ("zm" . reftex-reference)
          ("zj" . brust-LaTeX-insert-up)
          ("zk" . brust-LaTeX-insert-down)
          ("zñ" . brust-LaTeX-insert-backslash)
          ("e3" . reftex-toc)
          ("q1" . latex/clean-fill-indent-environment)
          ("w2" . LaTeX-env-brust))
    :bind 
    (:map LaTeX-mode-map
