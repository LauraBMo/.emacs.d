...
#+begin_src elisp
  ...
  (defface my/mode:vc-added
    `((((class color)) (:background "#FFAA55"  :foreground "black"))
    ...
    (let* ((state-branch
            (cond
             ...
             ((string-match "\\` ?\\(?:CVS\\|Git\\)\\([-:@]\\)\\([^^:~ \x00-\x1F\\\\/]+\\)?" vc-mode)
              (cons (match-string-no-properties 1 vc-mode) (concat (match-string-no-properties 2 vc-mode) " ")))
             ;; Otherwise, indicate confusion
             (t '("?" . ""))))
           ;; Fetch properties list for the class character above
           (props (cdr (assoc (car state-branch) my-vc-mode-attrs))))
           (concat (propertize (car props) 'face (cdr props)) 
                   " " (cdr state-branch))))
#+end_src
...
#+BEGIN_SRC elisp
  (setq-default
   ...
   '("%e"
     ...
     (:propertize "||  " face mode-line-bars-face)
     (:propertize display-time-string face sml/line-number)
     ;;mode-line-misc-info
     (:propertize " || " face mode-line-bars-face)
     (:eval
     ...
     mode-line-buffer-identification
     (:propertize "  |[  " face mode-line-bars-face)
     (:propertize ("" "%[" mode-name "%]") face sml/modes)
     ;; mode-line-modes
     ;;(:propertize "| " face mode-line-bars-face)
     ;; (vc-mode vc-mode)
     ;; (vc-mode vc-status)
...
#+RESULTS:
| %e | (:propertize |   | face mode-line-bars-face) | (:propertize display-time-string face sml/line-number) | (:propertize |   | face mode-line-bars-face) | (:eval (cond (buffer-read-only (propertize  RO  (quote face) (quote mode-line-read-only-face))) ((buffer-modified-p) (propertize  **  (quote face) (quote mode-line-modified-face))))) | (:eval (if (or buffer-read-only (buffer-modified-p)) (propertize   (quote face) (quote mode-line-bars-face))       )) | (:propertize | [ face mode-line-bars-face) | (:propertize %l face sml/line-number) | (:eval (propertize (save-excursion (goto-char (point-max)) (format-mode-line /%l)) (quote face) (quote mode-line-top-line-number))) | (:propertize : face sml/line-number) | (:eval (propertize %c (quote face) (if (< (current-column) 80) (quote sml/col-number) (quote mode-line-80col-face)))) | (:eval (propertize (format /%s (- (line-end-position) (line-beginning-position))) (quote face) (quote mode-line-top-line-number))) | (:propertize ] | face mode-line-bars-face) | mode-line-frame-identification | mode-line-buffer-identification | (:propertize | [   face mode-line-bars-face) | (:propertize ( %[ mode-name %]) face sml/modes) | (:propertize   ] |   |   | face mode-line-bars-face) | (:eval (nyan-create)) | (:propertize |   | face mode-line-bars-face) | (:eval (my-mode-line-vc-info)) | (:propertize |   | %- face mode-line-bars-face) | mode-line-end-spaces |
...
**** Load it
 ...
 #+begin_src elisp
   (require 'ess-site)
   (setq inferior-julia-program-name "~/scr/julia-1.2.0/bin/julia")
...
***** Headers
#+begin_src elisp
  (defun brust-mu4e-rebuild-index (msg)
    (async-shell-command "~/scr/mu/mu/mu index --rebuild"))
...
#+RESULTS:
: ((capture message . mu4e-action-capture-message) (show this thread . mu4e-action-show-thread) (Rebuild . brust-mu4e-rebuild-index))
**** Marks
#+begin_src elisp
  ...
  (add-to-list 'mu4e-marks
               '(tag
                 :char       "g"
                 :prompt     "gtag"
                 :ask-target (lambda () (ivy-read "What tag do you want to add? " (with-temp-buffer
                                                                                    (insert-file-contents "~/.emacs.d/brust-mu4e-tags-list-file.txt")
                                                                                    (split-string (buffer-string) "\n"))
                                                  :action (lambda (-str) (unless (member -str (with-temp-buffer
                                                                                                (insert-file-contents "~/.emacs.d/brust-mu4e-tags-list-file.txt")
                                                                                                (split-string (buffer-string) "\n")))
                                                                           (with-temp-buffer
                                                                             (insert-file-contents "~/.emacs.d/brust-mu4e-tags-list-file.txt")
...
**** My Contexts
 ...
 #+RESULTS:
 | #s(mu4e-context gmail (lambda nil (mu4e-message Entering context laurea987@gmail.com)) (lambda nil (mu4e-message Leaving context laurea987@gmail.com)) (lambda (msg) (when msg (mu4e-message-contact-field-matches msg (quote (:from :to :cc :bcc)) laurea987@gmail.com))) ((user-mail-address . laurea987@gmail.com) (user-full-name . Laura Brustenga) (mu4e-sent-folder . /gmail/Sent) (mu4e-drafts-folder . /gmail/Drafts) (mu4e-trash-folder . /gmail/Trash) (mu4e-compose-signature) (mu4e-compose-format-flowed))) | #s(mu4e-context mathuab (lambda nil (mu4e-message Entering context brust@mat.uab.cat)) (lambda nil (mu4e-message Leaving context brust@mat.uab.cat)) (lambda (msg) (when msg (mu4e-message-contact-field-matches msg (quote (:from :to :cc :bcc)) brust@mat.uab.cat))) ((user-mail-address . brust@mat.uab.cat) (user-full-name . Laura Brustenga i Moncusí) (mu4e-sent-folder . /mathuab/Sent) (mu4e-drafts-folder . /mathuab/Drafts) (mu4e-trash-folder . /mathuab/Trash) (mu4e-compose-signature) (mu4e-compose-format-flowed))) |
...
**** Bookmarkstr
 #+BEGIN_SRC elisp
   ...
   (setq mu4e-bookmarks
         `(
           ,(make-mu4e-bookmark
             :name "All Inboxes"
             :query "maildir:/mathuab/INBOX OR maildir:/Gmail/Inbox AND flag:unread"
             :key ?i)
           ,(make-mu4e-bookmark
             :name "New mathuab"
           ...
           ,(make-mu4e-bookmark
             ...
             :key ?u)
           ,(make-mu4e-bookmark
             :query "date:today..now AND NOT flag:trashed AND NOT flag:draft"
             :name "Today's messages"
             :key ?t)
           ,(make-mu4e-bookmark
             :query "date:7d..now AND NOT flag:trashed AND NOT flag:draft"
             :name "Last 7 days"
             :key ?w)
 ...
 #+RESULTS:
 | #s(mu4e-bookmark Unread messages flag:unread AND NOT flag:trashed 117) | #s(mu4e-bookmark Today's messages date:today..now AND NOT flag:trashed AND NOT flag:draft 116) | #s(mu4e-bookmark Last 7 days date:7d..now AND NOT flag:trashed AND NOT flag:draft 119) | #s(mu4e-bookmark All Inboxes maildir:/mathuab/INBOX OR maildir:/Gmail/Inbox 105) | #s(mu4e-bookmark New mathuab flag:unread AND maildir:/mathuab/INBOX 109) | #s(mu4e-bookmark Drafts flag:draft 100) |
