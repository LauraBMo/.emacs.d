...
#+BEGIN_SRC elisp
  (use-package org-ref
    ...
    (defvar brust-arxiv-lengh-base-web)
    (setq  brust-arxiv-lengh-base-web 22)
    (defun brust-arxiv-get-number-from-web (arxiv-web)
      (with-temp-buffer
        (insert arxiv-web)
        (delete-region (point-min) (+ (point-min) brust-arxiv-lengh-base-web))
        (buffer-string)))

    (defun brust-arxiv-get-pdf-add-bibtex-entry (arxiv-web)
