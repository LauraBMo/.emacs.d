...
*** Not edit minibuffer prompt
    Stop Cursor Going into Minibuffer Prompt, from http://ergoemacs.org/emacs/emacs_stop_cursor_enter_prompt.html
#+BEGIN_SRC elisp
...
#+END_SRC
*** Answear any question with y or n
#+BEGIN_SRC elisp
...
*** Refresh before installing
    If the installation of some packages is required, then run =(package-refresh-contents)= before insatalling the first.
#+BEGIN_SRC elisp
...
#+END_SRC
*** Open fast regualr fiels (by Xah Lee)
**** Lists
#+BEGIN_SRC elisp
  ...
  (setq
   ...
   '((xah-filelist . (("Th-master"       . "~/Dropbox/Doctorat_Laura/Thesis/master.tex")
                      ("My-notes"        . "~/Dropbox/Doctorat11/Apunts/basic-algebraic-geometry.tex")
                      ("Com-in-Alg"      . "~/Dropbox/Doctorat_Laura/Com_in_Alg/PauBrustenga_Urcf4.tex")
                      ("Org-personal"    . "~/Dropbox/Org/my.org")
                      ("Recetari"        . "~/Dropbox/Org/recetari.org")
                      ("Eng-writing"     . "~/Dropbox/English/Writings.tex")
                      ("Eng-read"        . "~/Dropbox/English/Readings.tex")))
     (config-files . (("Org-init"        . "~/.emacs.d/init.org")
...
#+BEGIN_SRC elisp
  (bind-keys*
   ...
   ("C--"         . undo)
   ("C-x C-F"     . xah-open-in-external-app)
...
#+BEGIN_SRC elisp
  (bind-keys*
   ("C-M-j" . outline-hide-more)
   ("C-M-l" . outline-show-more)
   ("C-M-h" . outline-up-heading)                ; Up
   ("C-M-k" . outline-next-visible-heading)      ; Next
   ("C-M-i" . outline-previous-visible-heading)  ; Previous
   ("C-M-S-K" . outline-forward-same-level)        ; Forward - same level
   ("C-M-S-I" . outline-backward-same-level))      ; Backward - same level
...
#+BEGIN_SRC elisp
  (use-package company
    ...
    (defun brust-company-abort-insert-spc ()
      (interactive)
      (company-abort)
      (insert " "))
    (defun brust-company-complete-selection-and-continue nil
      (interactive)
      (company-complete-selection))
    :bind (:map company-active-map
                ("C-<return>"   . company-complete-common-or-cycle)  
                ("<return>"     . company-complete-selection)
                ("M-<return>"   . brust-company-complete-selection-and-continue)
                ("TAB"          . company-abort)
                ("<tab>"        . company-abort)
                ...
                ("SPC"          . brust-company-abort-insert-spc))
    :config 
    (global-company-mode)
...
#+BEGIN_SRC elisp
  (use-package tex 
    ...
    :init
    (brust-endless/org-eval-eblocks "~/.emacs.d/lisp/brusts-latex-config.org" "LaTeX-init" t)
    ...
    (defun brust-LaTeX-insert-math2 nil
      (interactive) (insert "\n\\[\n  ?\n\\] ") (cdlatex-position-cursor))
    :config
    (brust-endless/org-eval-eblocks "~/.emacs.d/lisp/brusts-latex-config.org" "LaTeX-config" t)
...
** Discover
    To dicover emacs functionality.
#+BEGIN_SRC elisp
...
#+BEGIN_SRC elisp
    (use-package embrace
      :bind* ("M-ñ". embrace-commander)
      :chords ("zz" . embrace-commander))
#+END_SRC
...
(use-package multiple-cursors
  ...
  ("C-c C-x C-<" . mc/mark-all-like-this))
#+END_SRC
...
#+BEGIN_SRC elisp
  (use-package bongo
    ...
    (setq bongo-default-directory "~/Music"
          bongo-default-library-buffer-name "Bongo-Library"
          bongo-default-playlist-buffer-name "Bongo-Playlist"
          bongo-enabled-backends (quote (vlc))
          bongo-insert-whole-directory-trees t
          bongo-mark-played-tracks t
          bongo-next-action (quote bongo-play-random-or-stop)
          bongo-player-finished-hook nil)
    (defun brust-init-bongo (args)
      (interactive "P")
      (bongo)
      (bongo-insert-file "~/Music"))
    :bind ("<f9>" . brust-init-bongo))
#+END_SRC
...
** Org Mobile
  ...
  (use-package org-mobile
    ...
    :config
      (setq org-mobile-directory "~/Dropbox/config/MobilOrg")
      (setq org-mobile-inbox-for-pull "~/Dropbox/config/MobilOrg/mobileorg.org")
      (setq default-buffer-file-coding-system 'utf-8)
      (setq org-mobile-files '("~/Dropbox/config/MobilOrg/Merian.org"
                               "~/Dropbox/config/MobilOrg/my.org"))
...
* Not usual packages
  :TODO:     create functions to load: maxima, singular, teaching, my-dicctionary,..
  and not load them on any startup
...
#+BEGIN_SRC elisp
  ...
  (defun brust-my-dictionary-open-an-entry (header)
    (find-file (concat "~/Dropbox/English/My-dictionary/sections/"
  ...
  (defun brust-my-dictionary-new-entry (header)
    ...
    (insert-file-contents "~/Dropbox/English/My-dictionary/tamplate-new-entry.tex")
    (goto-char (point-min))
       ...
       (completing-read
        ...
        (cl-remove-duplicates
         (mapcar (lambda (x) (replace-regexp-in-string "[-]" " " (file-name-base x)))
                 (directory-files-recursively "~/Dropbox/English/My-dictionary/sections" ".+[.]tex"))
